...
const mapStatetoProps = (state: RootState) => { // store 的注入
    return {
...
const mapDispatchToProps = (dispatch: Dispatch) => { // store 输入
    return {
        changeLanguage: (code: 'zh' | 'en') => {
            const action = changeLanguageActionCreate(code)
            dispatch(action)
        },
        addLanguage: (name: string, code: string) => {
            const action = addLanguageActionCreate(name, code)
            dispatch(action)
        }
    }
}
...
type PropsType = RouteComponentProps & // react-router 路由props类型
    WithTranslation & // react-i18next props类型
    ReturnType<typeof mapStatetoProps> & // 函数的类型需要反向注入，使用 Return<> /  redux store 映射类型
        ...
        return (
            <div className={styles['app-header']}>
                ...
                <Layout.Header className={styles['main-header']}>
                    ...
                    {/* antd自带的Input */}
                    <Input.Search onSearch={keywords => history.push(`/search/${keywords}`)} className={styles['search-input']} placeholder={t('header.search_placeholder')}></Input.Search>
                </Layout.Header>
